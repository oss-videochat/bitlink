...
class RoomService {
    ...
    static broadcastHosts(room: Room, events: string, ...args: any[]) {
        const nonHosts: Participant[] = RoomService.getConnectedParticipants(room).filter(
            (participant: Participant) => participant.role !== ParticipantRole.HOST
        );
    ...
    static broadcast(
        room: Room,
        event: string,
        ignoreParticipants: Participant[] = [],
        ...args: any[]
    ) {
    ...
    static getSummary(room: Room, currentParticipant: Participant): RoomSummary {
        return {
            ...
            idHash: room.idHash,
            name: room.settings.name,
            participants: room.participants.map((participantInRoom) =>
                ParticipantService.getSummary(participantInRoom)
            ),
            myId: currentParticipant.id,
            messages: room.messages
                .filter((message) =>
                    MessageService.hasPermissionToView(message, currentParticipant)
                )
    ...
    static closeRoomIfNecessary(room: Room) {
        if (
            room.participants.filter(
                (participant) =>
                    participant.isConnected && participant.role === ParticipantRole.HOST
            ).length === 0
        ) {
            if (
                room.settings.hostDisconnectAction === HostDisconnectAction.TRANSFER_HOST &&
                room.participants.filter((participant) => participant.isConnected).length > 0
            ) {
                RoomService.changeRole(
                    room,
                    room.participants.filter((participant) => participant.isConnected)[0],
    ...
    static waitingRoomDecision(
        ...
        const waitingRoomIndex = room.waitingRoom.findIndex(
            (patientParticipant) => patientParticipant.id === id
        );
        ...
        switch (decision) {
            ...
            case "reject":
                patientParticipant.socket.emit("waiting-room-rejection", {
                    reason: "The host rejected you.",
                });
    ...
    static updateRoomSettings(room: Room, newSettings: RoomSettings) {
        ...
        if (
            JSON.stringify(RoomService._getSafeSettings(room.settings)) !==
            JSON.stringify(safeVersion)
        ) {
            this.broadcast(
                room,
                "updated-room-settings",
                RoomService.getConnectedParticipants(room).filter(
                    (participant) => participant.role === ParticipantRole.HOST
                ),
                { newSettings: safeVersion }
            );
        }
    ...
    static sendSystemMessage(
        room: Room,
        content: string,
        permission: ParticipantRole = ParticipantRole.MEMBER
    ) {
    ...
    static alertRelevantParticipantsAboutMessage(
        room: Room,
        message: Message,
        eventType: "new" | "edit" | "delete"
    ) {
        ...
        switch (message.type) {
            case MessageType.SYSTEM: {
                RoomService.broadcast(
                    ...
                    `${eventType}-message`,
                    RoomService.getConnectedParticipants(room).filter(
                        (participant) => participant.role > (message as SystemMessage).permission
                    ),
            ...
            case MessageType.GROUP: {
                (message as GroupMessage).group.members
                    .filter((participant) => participant.isConnected)
                    .forEach((participant) => {
                        participant.socket.emit(`${eventType}-message`, {
                            messageSummary: summary,
                        });
                    });
                break;
    ...
    static transferHost(room: Room, from: Participant, to: Participant) {
        ...
        this.broadcast(room, "participant-update-role", [], {
            participantId: to.id,
    ...
    static changeRole(room: Room, participant: Participant, role: ParticipantRole) {
        ...
        RoomService.sendSystemMessage(
            room,
            `${participant.name}'s role is now ${role === ParticipantRole.HOST ? "host" : "member"}`
        );
    }
    ...
    private static _addParticipant(room: Room, participant: Participant) {
        ...
        function pw(
            func: handleParticipantEvent<any>,
            validation?: ((data: any) => boolean) | object
        ...
        participant.socket.on(
            "producer-action",
            pw(Handlers.handleProducerAction, Validation.handleProducerAction)
        );
        participant.socket.on("get-room-settings", pw(Handlers.handleGetRoomSettings));
        participant.socket.on(
            "update-room-settings",
            pw(Handlers.handleUpdateRoomSettings, Validation.handleUpdateRoomSettings)
        );
        participant.socket.on(
            "kick-participant",
            pw(Handlers.handleKickParticipant, { participantId: { type: "string" } })
        );
        participant.socket.on("send-message", pw(Handlers.handleSendMessage));
        participant.socket.on(
            "edit-message",
            pw(Handlers.handleEditMessage, {
        ...
        participant.socket.on(
            "delete-message",
            pw(Handlers.handleDeleteMessage, { messageId: { type: "string" } })
        );
        participant.socket.on(
            "transfer-host",
            pw(Handlers.handleTransferHost, { participantId: { type: "string" } })
        );
        participant.socket.on(
            "change-name",
            pw(Handlers.handleUpdateName, { newName: { type: "string" } })
        );
        ...
        RoomService.broadcast(room, "new-participant", [participant], {
            participantSummary: ParticipantService.getSummary(participant),
        });
