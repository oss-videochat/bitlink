...
class ParticipantService {
  static getLiving(excludeSelf = false) {
    return ParticipantsStore.participants.filter(
      (participant) =>
        participant.info.isAlive &&
        (excludeSelf ? participant.info.id !== MyInfo.participant?.id : true)
    );
  }
  ...
  static getById(id: string) {
    return ParticipantsStore.participants.find(
      (participant: Participant) => participant.info.id === id
    );
  }

  static getIndexById(id: string) {
    return ParticipantsStore.participants?.findIndex(
      (participant: Participant) => participant.info.id === id
    );
  }

  static removeFromWaitingRoom(id: string) {
    const waitingRoomIndex: number = ParticipantsStore.waitingRoom.findIndex(
      (patientParticipant) => patientParticipant.info.id === id
    );
  ...
  static filterByMentionString(mentionString: string): Participant[] {
    ...
    return ParticipantsStore.participants.filter(
      (participant) => participant.mentionString.toLowerCase().startsWith(mentionString) // we need to sort
    );
  }
  ...
  static getByMentionString(mentionString: string) {
    ...
    return ParticipantsStore.participants.filter(
      (participant) => participant.mentionString.toLowerCase() === mentionString
    );
  }
}
