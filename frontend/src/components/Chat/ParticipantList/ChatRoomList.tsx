...
const ChatRoomList: React.FunctionComponent<IChatParticipantListProps> = ({
    selectedRoom,
    onRoomSelect,
}) => {
    ...
    return useObserver(() => (
        <div
            className={"chat-participant-wrapper " + (UIStore.store.participantPanel ? "open" : "")}
        >
            <div className={"chat-participant-wrapper--content"}>
                <SearchBar onChange={handleSearchChange} />
                {RoomStore.info ? (
                    <div className={"chat-participant-list"}>
                        {ParticipantsStore.waitingRoom.length > 0 ? <WaitingRoomList /> : null}
                        {RoomStore.groups.map((group) => (
                            <ChatParticipant
                                onChosen={onChosen}
                                selected={
                                ...
                                key={"chat-" + group.id}
                                lastMessage={ChatStoreService.getLatestMessage(
                                    MessageType.GROUP,
                                    group.id
                                )}
                        ...
                        {ParticipantsStore.participants
                            ...
                            .map((participant) => {
                                const lastMessage = ChatStoreService.getLatestMessage(
                                    MessageType.DIRECT,
                                    participant.info.id
                                );
                                ...
                                return (
                                    <ChatParticipant
                                        onChosen={onChosen}
                                        selected={
                                            selectedRoom.type === MessageType.DIRECT &&
                                            selectedRoom.id === participant.info.id
                                        }
                                        key={"chat-" + participant.info.id}
