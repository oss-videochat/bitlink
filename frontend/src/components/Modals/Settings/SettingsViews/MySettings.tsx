...
const MySettings: React.FunctionComponent<ISettingsPanelProps> = ({
    events,
    changesMade,
    handleChangesMade,
}) => {
    ...
    return (
        <div className={"settings-view"}>
                ...
                >
                    {deviceList
                        ...
                        .map((devicesInfo: MediaDeviceInfo, index: number) => (
                            <option key={devicesInfo.deviceId} value={devicesInfo.deviceId}>
                                {devicesInfo.label || "camera " + (index + 1)}
                            </option>
                        ))}
                </select>
            </label>
            ...
            <label>
                Audio Input:
                <select
                    onChange={(e) => handleInputChange("audio", e)}
                    className={"modal--select"}
                ...
                >
                    {deviceList
                        ...
                        .map((devicesInfo: MediaDeviceInfo, index: number) => (
                            <option key={devicesInfo.deviceId} value={devicesInfo.deviceId}>
                                {devicesInfo.label || "microphone " + (index + 1)}
                            </option>
                        ))}
                </select>
            </label>
