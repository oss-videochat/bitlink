...
class IO {
        ...
        reaction(
            ...
            async (_) => {
                ...
                if (
                    ...
                    MyInfo.producers.microphone.track?.getSettings().deviceId !==
                        MyInfo.preferredInputs.audio
                ) {
                    log("Preferred audio input changed detected");
                    HardwareService.getStream("microphone").then((stream) => {
                        MyInfo.producers.microphone!.replaceTrack({
                ...
                if (
                    ...
                    MyInfo.producers.camera.track?.getSettings().deviceId !==
                        MyInfo.preferredInputs.video
                ) {
    ...
    joinRoom(id: string, name?: string) {
        ...
        this.socketRequest("get-rtp-capabilities", { roomId: id })
            .then((response: APIResponse) => {
                ...
                if (isSafari) {
                    response.data.headerExtensions = response.data.headerExtensions.filter(
            ...
            .then(() => {
                this.io.emit(
                    ...
                    (response: APIResponse) => {
                        ...
                        this.createTransports().then(() => this.io.emit("transports-ready"));
                        setTimeout(() => {
    ...
    createTransports() {
        ...
        const addTransportListeners = (transport: mediasoupclient.types.Transport) => {
            ...
            transport.on("connect", async ({ dtlsParameters }, callback, errback) => {
                ...
                if (!response.success) {
                    ...
                    NotificationService.add(
                        NotificationService.createUINotification(
                            `An error occurred connecting to the transport: ${response.error}`,
                            NotificationType.Error
                        )
                    );
                    errback();
            ...
            transport.on("produce", async ({ kind, rtpParameters, appData }, callback, errback) => {
                ...
                try {
                    ...
                    this.socketRequest("producer-action", {
                        source: appData.source,
                        action: "resume",
                    });
        ...
        return Promise.all([
            this.socketRequest("create-transport", {
                ...
                MyInfo.transports.receiving = RoomStore.device!.createRecvTransport(
                    response.data.transportInfo
                );
                return addTransportListeners(MyInfo.transports.receiving);
            }),
            this.socketRequest("create-transport", { type: "webrtc", kind: "sending" }).then(
                (response: APIResponse) => {
                    MyInfo.transports.sending = RoomStore.device!.createSendTransport(
                        response.data.transportInfo
                    );
    ...
    async toggleMedia(source: MediaSource) {
        ...
        if (!stream) {
            NotificationService.add(
                NotificationService.createUINotification(
                    `An error occurred accessing the ${source}`,
                    NotificationType.Error
                )
            );
            return;
    ...
    async send(messageType: MessageType, toId: string, content: string) {
        ...
        if (!response.success) {
            NotificationService.add(
                NotificationService.createUINotification(
                    `An error occurred sending the message: "${response.error}"`,
                    NotificationType.Error
                )
            );
    ...
    async edit(messageId: string, content: string) {
        const response = await this.socketRequest("edit-message", {
            messageId: messageId,
            newContent: content,
        });
        ...
        if (!response.success) {
            NotificationService.add(
                NotificationService.createUINotification(
                    `An error occurred editing the message: "${response.error}"`,
                    NotificationType.Error
                )
            );
    ...
    async delete(messageId: string) {
        ...
        if (!response.success) {
            NotificationService.add(
                NotificationService.createUINotification(
                    `An error occurred deleting the message: "${response.error}"`,
                    NotificationType.Error
                )
            );
    ...
    async waitingRoomDecision(id: string, accept: boolean) {
        ...
        if (!response.success) {
            NotificationService.add(
                NotificationService.createUINotification(
                    `An error occurred while deciding on waiting room member: "${response.error}"`,
                    NotificationType.Error
                )
            );
    ...
    async getRoomSettings(): Promise<RoomSettings> {
        ...
        if (!response.success) {
            NotificationService.add(
                NotificationService.createUINotification(
                    "Error Getting Settings: " + response.error,
                    NotificationType.Error
                )
            );
            throw response.error;
    ...
    async kick(participant: Participant): Promise<void> {
        const response = await this.socketRequest("kick-participant", {
            participantId: participant.info.id,
        });
        if (!response.success) {
            NotificationService.add(
                NotificationService.createUINotification(
                    "Error Kicking Participant: " + response.error,
                    NotificationType.Error
                )
            );
            throw response.error;
    ...
    processRoomSummary(data: { summary: RoomSummary; rtcCapabilities: any }) {
        ...
        const participants = roomSummary.participants.map(
