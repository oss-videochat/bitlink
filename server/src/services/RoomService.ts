...
class RoomService {
  ...
  static closeRoomIfNecessary(room: Room) {
    if (
      room.participants.filter(
        (participant) => participant.isConnected && participant.role === ParticipantRole.HOST
      ).length === 0
    ) {
      if (
  ...
  static updateRoomSettings(room: Room, newSettings: RoomSettings) {
    ...
    if (
      JSON.stringify(RoomService._getSafeSettings(room.settings)) !== JSON.stringify(safeVersion)
    ) {
      this.broadcast(
        room,
