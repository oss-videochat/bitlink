...
export const Grid: React.FunctionComponent<GridProps> = ({container}) => {
    const styles = useLayoutCalculation(() => {
        ...
        return living.filter(participant => participant.hasAudio || participant.hasVideo).length + living.filter(participant => participant.hasScreen).length;
    }, container);
                    ...
                    participantsMedia.map((participant, i, arr) => (
                        <TileWrapper menuItems={[{
                            ...
                            toggle: () => UIStore.store.layout = {mode: TileDisplayMode.PINNED_PARTICIPANT, participant}
                                     ...
                                     key={participant.info.id + participant.hasVideo} style={styles}>
