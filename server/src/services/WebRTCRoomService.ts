...
class WebRTCRoomService {
    static async createTransport(room: Room, participant: Participant, type: TransportType, kind: TransportJob): Promise<types.WebRtcTransport> {
        log("Participant creating a transport %s:%s", participant.name, kind);
        let transport;
        switch (type) {
            case "webrtc":
                transport = await room.router.createWebRtcTransport(config.mediasoup.webRtcTransportOptions as WebRtcTransportOptions);
                break;
            case "plain":
                throw "Unsupported";
    ...
    static async createProducer(room: Room, participant: Participant, transport: types.Transport, kind: MediaType, rtpParameters: types.RtpParameters, source: MediaSource) {
    ...
    private static async createConsumerAndNotify(room: Room, producerPeer: Participant, consumerPeer: Participant, source: MediaSource) {
