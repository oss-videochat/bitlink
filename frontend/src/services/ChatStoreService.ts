...
class ChatStoreService {
    ...
    static getMessages(type: MessageType, roomId: string) {
        return ChatStore.messageStore.filter((aMessage) => {
            let idMatch = false;
            if (
                aMessage.type === MessageType.GROUP &&
                (aMessage as GroupMessage).group.id === roomId
            ) {
                idMatch = true;
            }
            if (
                aMessage.type === MessageType.DIRECT &&
    ...
    static editNextMessage(type: MessageType, roomId: string, message?: Message) {
        ...
        if (!message) {
            UIStore.store.messageIdEditControl =
                messagesInChat[messagesInChat.length - 1]?.id || null;
            return;
        }
        const nextEditableMessage = ChatStoreService.getNextEditableMessage(
            messagesInChat.slice().reverse(),
            message
        );
    ...
    private static getNextEditableMessage(messages: Message[], message: Message) {
        ...
        return messages.find((messageInChat) => {
            if (
                messageInChat.type !== MessageType.DIRECT &&
                messageInChat.type !== MessageType.GROUP
            ) {
                // you can't edit a system message
                return false;
            }
            const userMessageInChat = messageInChat as GroupMessage | DirectMessage;
            if (hit && userMessageInChat.from.info.id === MyInfoStore.participant!.id) {
                // must be editable
                return true;
            }
