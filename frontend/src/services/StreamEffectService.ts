...
class StreamEffectService {
    static async enableBlur(){
        ...
        if(StreamEffectStore.cameraStreamEffectRunner){
            ...
            StreamEffectStore.cameraStreamEffectRunner = await CameraStreamEffectsRunner.create(await HardwareService.getRawStream("camera"), true);
        }
    }
    ...
    static async enableVirtualBackground(image: HTMLImageElement){
        ...
        if(StreamEffectStore.cameraStreamEffectRunner){
            ...
            StreamEffectStore.cameraStreamEffectRunner = await CameraStreamEffectsRunner.create(await HardwareService.getRawStream("camera"), false, image);
        }
    }
    ...
    static async generateNewEffectRunner(){
        ...
        if(StreamEffectStore.image || StreamEffectStore.blur){
            StreamEffectStore.cameraStreamEffectRunner = await CameraStreamEffectsRunner.create(await HardwareService.getRawStream("camera"),    StreamEffectStore.blur, StreamEffectStore.image || undefined);
        } else {
            StreamEffectStore.cameraStreamEffectRunner = null;
        }
    }
